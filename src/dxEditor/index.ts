 ...
 * @LastEditTime: 2024-12-16 11:11:07
 */
...
export class EditorView extends EventDispatcher {
  ...
  private onMessage(event: MessageEvent) {
    ...
    //   console.log('Message from untrusted source:', event.origin)
    //   return
    // }

    // 处理接收到的消息
    console.log('子页面接收消息:', event.data)
    this.exportJson(event.data.children)
    this.keybord.hotkeys.showAll()
    // 根据消息内容执行相应操作
    switch (event.data.type) {
      case 'update':
        this.updateContent(event.data.payload)
        break
      case 'command':
  ...
  // 示例方法：更新内容
  private updateContent(payload: any) {
    // 更新 iframe 内容的逻辑
