 ...
 * @LastEditTime: 2024-12-13 09:29:59
 */
...
const PicAssets: React.FC<Props> = ({ className = '' }) => {
  ...
  const [imgs, setImgs] = useState<Children[]>(imgData.find((data) => data.name === selected)?.children || [])
      ...
      <div
        ...
        {imgs.map((img) => {
          return (
            <div
              key={img.id}
              className="w-80px h-80px p-6px box-border cursor-pointer border-1px hover:border-#666 rounded-6px border-#fff flex flex-col justify-between"
              ...
              />
              <div className="max-w-70px text-center" style={{ font: '16px arial, sans-serif' }}>
                {img.name}
              </div>
            </div>
          )
